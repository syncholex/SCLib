...
if (true)
 {
	 // Hook all methods in android.os.SystemProperties

const targetClass = "android.os.SystemProperties";

setImmediate(() => {
  Java.perform(() => {
    try {
      // Use the 'android.os.SystemProperties' class
      const SystemProperties = Java.use("android.os.SystemProperties");

      // Hook the 'get' method
      SystemProperties.get.overload('java.lang.String').implementation = function (key) {
        const value = this.get(key); // Call the original method

        // Check if the accessed property is 'ro.build.fingerprint'
        if (key === 'ro.build.fingerprint') {
          console.log(`ro.build.fingerprint: ${value}`);
        }
    ...
    Java.perform(function () {
        ...
        /******** bypass ssl in java layer, version 12.55 ***************/
        if (true) {
            console.log('')
            console.log('===')
            ...
            try {
                var CertificatePinner = Java.use("hy2");//("Zb2"); //search by "sha256/"  //("W92");// Java.use("C72");// Java.use("W92");// Java.use("C72");// Java.use("Y32"); // Java.use("tux"); //Java.use("rfx"); // Java.use("SIv");
                CertificatePinner.a.overload('java.lang.String', 'java.util.List').implementation = function (host, list) {
                    console.log("CertificatePinner.check for host : " + host);
                    return null;
                ...
                console.log("wrong in certificate pinner hook : ", e)
            }


            try {
                /**** important. in different app versions, implementation this method can be different *************/
                var CronetPinning = Java.use("Mh5");//("dI4");//("Ly4");// search by "DISABLE_PINNING))" // Java.use("cu4");// Java.use("My4");// Java.use("cu4"); // Java.use("yl4"); // Java.use("enm"); // Java.use("Zbm"); //Java.use("n2l");
                CronetPinning.invoke.overload().implementation = function () {
                    console.log("Cronet pinning hook");
                    var a = this.d.value;
                    console.log("Cronet pinning hook a = " + a);
                    if (a == 0) {
                        console.log("Cronet pinning hook with pinning : " + a);
                        return null;
                    } else {
                        return this.invoke();
                    }
                ...
                console.log("wrong in cronet pinner hook : ", e)
            }
        ...
        console.log("Finish Snapchat script");
    });
}
        ...
        Java.perform(function () {
            ...
            console.log('======');
                ...
                try {
                    ...
                    TrustManagerImpl_Activity_1.qwaazxdffttrbgf.implementation = function (certs, ocspData, tlsSctData, host, clientAuth, untrustedChain, trustAnchorChain, used) {
